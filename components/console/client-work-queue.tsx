...
function RankedQueue({ clientId, data, state }: { clientId: string; data: ClientWorkData; state: ChecklistRenderState }) {
  ...
  const rank = (row: typeof rows[number]) => row.priority === 0 ? -1 : row.dueAt && day(row.dueAt) < today ? 0 : row.dueAt && day(row.dueAt) === today ? 1 : row.priority === 1 ? 2 : 3;
  ...
  const visible = rows.filter(row => filter === "all" || (filter === "work" ? (row.kind === "work" && row.item.state === "needs_you") || row.kind === "manual" : row.kind === filter));
  ...
  return <div className="space-y-5">
    ...
    <div id="work-list" className="scroll-mt-24"><span id="all" className="block scroll-mt-24" /><span id="requests" className="block scroll-mt-24" /><span id="monitoring" className="block scroll-mt-24" /><ul className="overflow-hidden rounded-xl border border-sand bg-warm-white shadow-sm">{visible.map(row => {
      ...
      if (row.kind === "work") {
        ...
        return <WorkItemRow key={`work-${row.id}`} id={`work-item-${row.id}`} title={portalCopy(item.title)} priority={item.priority} timing={timing} action={primary}><p>{portalCopy(item.why)}</p><ol className="mt-3 list-decimal space-y-2 pl-5">{item.instructions.map((step, index) => <li key={index}>{portalCopy(step)}</li>)}</ol><div className="mt-4 flex flex-wrap gap-3">{item.canSnooze && <button className={button} disabled={state.pending !== null} onClick={() => void state.acknowledge(item, "snooze")}>Snooze {item.defaultSnoozeDays ?? 14} days</button>}{item.action && item.canMarkDone && <button className={button} disabled={state.pending !== null} onClick={() => void state.acknowledge(item, "done")}>Mark done</button>}{(item.action || item.canMarkDone) && item.href && <Link className={button} href={workItemHref(item.href, clientId)}>Open related record</Link>}</div></WorkItemRow>;
      }
      if (row.kind === "manual") return <WorkItemRow key={`manual-${row.id}`} title={portalCopy(row.item.title)} timing={timing} action={<button className={button} disabled={state.pending !== null} onClick={() => void state.updateManual(row.item, "toggle")}>Mark done</button>}><p>{row.item.details ?? "Added by the team."}</p><button className={`${button} mt-3`} disabled={state.pending !== null} onClick={() => void state.updateManual(row.item, "delete")}>Remove item</button></WorkItemRow>;
      if (row.kind === "alerts") return <WorkItemRow key={`alert-${row.id}`} title={portalCopy(row.item.title)} detail="Monitoring alert" timing={timing} priority={row.priority as 0 | 1 | 2 | 3} action={<button className={button} disabled={pendingAlert !== null} onClick={() => void acknowledge(row.id)}>{pendingAlert === row.id ? "Acknowledging…" : "Acknowledge"}</button>}><p>{portalCopy(row.item.message)}</p><p className="mt-2 font-mono text-xs">Raised {day(row.item.created_at)}</p></WorkItemRow>;
      return <WorkItemRow key={`request-${row.id}`} title={portalCopy(row.item.title)} detail="Waiting on client" timing={timing} priority={row.priority as 0 | 1 | 2 | 3} action={row.item.request_type === "roster_collection" && row.item.upload_token ? <RosterLinkCopy compact url={`${data.appUrl}/roster/${row.item.upload_token}`} /> : undefined}><p>{portalCopy(row.item.description ?? "Awaiting the requested information.")}</p><p className="mt-2">Reminders: {row.item.reminder_count} / {row.item.reminder_limit}{row.item.next_reminder_at ? ` · Next ${day(row.item.next_reminder_at)}` : ""}{row.item.escalated_at ? " · Escalated" : ""}</p></WorkItemRow>;
