...
export default async function MonitoringPage({
  ...
  const [{ data: client, error: clientError }, snapshots, latestMonitoringRunResult, alertsResult] = await Promise.all([
    supabase
    .from("clients")
    .select("id")
    .eq("id", id)
  ...
  if (clientError) throw new Error(`Unable to load client monitoring record: ${clientError.message}`);
  if (!client) notFound();
