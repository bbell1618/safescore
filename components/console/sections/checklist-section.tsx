...
export default async function ClientChecklistPage({
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-6">
      <h2 className="mb-4 font-heading text-2xl text-navy">Work · {checklist.items.length + checklist.manualItems.filter(item => item.status === "open" && !item.deletedAt).length}</h2>
      <OperatorChecklist
        clientId={id}
