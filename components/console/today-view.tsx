      ...
      <div className="min-w-0 space-y-8">
        <section aria-labelledby="work-queue-heading"><h2 id="work-queue-heading" className="mb-4 font-heading text-2xl text-navy">Work queue</h2>
          {workCount === 0 ? <Empty title="Queue is clear" copy="There are no open work items for active subscription clients." /> : <div className="space-y-5">{groups.filter(group => group.items.length || group.manualItems.length).map(group => <article key={group.client.id} className="overflow-hidden rounded-xl border border-sand bg-warm-white shadow-sm">
            <header className="flex flex-wrap items-center gap-3 border-b border-sand bg-cream px-5 py-4"><h3 className="min-w-0 flex-1 font-heading text-xl text-navy">{group.client.name}</h3><Badge variant={tierBadgeVariant(group.client.tier)}>{tierDisplayLabel(group.client.tier)}</Badge><span className="font-mono text-sm text-amber-dark">{group.burden == null ? "No snapshot" : `${group.burden} burden pts`}</span></header>
            <ul>{group.items.map(item => <WorkItemRow key={item.id} title={item.title} priority={item.priority} href={workItemHref(item.href, group.client.id)} timing={item.timing} />)}
              {group.manualItems.map(item => <WorkItemRow key={item.id} title={item.title} href={`/console/clients/${group.client.id}/work#manual-items-heading`} timing={item.dueDate ? `Due ${formatDate(item.dueDate)}` : age(item.createdAt)} />)}</ul>
          </article>)}</div>}
        </section>
