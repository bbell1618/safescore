...
export function ConsolePageSkeleton({ shape = "table", hero = ["today", "profile", "plan", "account"].includes(shape) }: { shape?: Shape; hero?: boolean }) {
  return <div role="status" aria-label="Loading page" className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
    {hero ? <div className="portal-navy-texture rounded-2xl p-8"><div className="h-4 w-56 rounded bg-warm-white/20" /><div className="mt-4 h-14 w-40 rounded bg-warm-white/20" /><div className="mt-6 flex flex-wrap gap-3">{[0, 1, 2].map(i => <div key={i} className="h-14 w-40 rounded-lg bg-amber/20" />)}</div></div> : <div className="space-y-3 py-3"><Line className="h-8 w-56" /><Line className="w-80 max-w-full" /></div>}
