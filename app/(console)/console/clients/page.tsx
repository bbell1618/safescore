...
export default async function ConsolePage() {
  ...
  const clientsPromise = Promise.resolve(supabase.from("clients").select("*").order("created_at", { ascending: false }));
  const [clientsResult, alertsResult, clientBurdenEntries] = await Promise.all([
    clientsPromise,
    supabase
      .from("alerts")
      .select("client_id")
    ...
    clientsPromise.then(result => {
      if (result.error) throw new Error(`Unable to load console clients: ${result.error.message}`);
      return Promise.all((result.data ?? []).map(async client => [client.id, await getClientBurden(client.id)] as const));
    }),
  ]);
